...
export class ContainerService {
  static async verifyDockerConnection() {
    try {
      ...
      throw status(
        500,
        `Docker daemon is not reachable {${err}}` satisfies ContainerModel.response
      );
    }
  }
  ...
  private static async pullImage(imageName: string) {
    ...
    try {
      ...
      throw status(
        404,
        `Failed to pull image: ${imageName}; ${err}` satisfies ContainerModel.response
      );
    }
  }
  ...
  private static async ensureContainerIsManaged(containerId: string) {
    ...
    try {
      ...
      throw status(
        404,
        `Container with ID ${containerId} not found.` satisfies ContainerModel.response
      );
    }
    ...
    if (!labels['qube.server']) {
      throw status(
        403,
        `Container with ID ${containerId} is not managed by Qube.` satisfies ContainerModel.response
      );
    }
  }
  ...
  static async createContainer(
    params: ContainerModel.createContainerBody
  ): Promise<ContainerModel.containerInfo> {
    ...
    if (params.persistentDataPath)
      volumeBinds.push(`/app/server/${randomContainerId}:${params.persistentDataPath}`);
  ...
  static async startContainer(containerId: string) {
    ...
    try {
      ...
      throw status(
        404,
        `Failed to start container with ID: ${containerId}; ${err}` satisfies ContainerModel.response
      );
    }
    return this.getContainerInfo(container.id);
  }
  ...
  static async restartContainer(containerId: string) {
    ...
    try {
      ...
      throw status(
        404,
        `Failed to restart container with ID: ${containerId}; ${err}` satisfies ContainerModel.response
      );
    }
    return this.getContainerInfo(container.id);
  }
  ...
  static async stopContainer(containerId: string, force: boolean = false) {
    ...
    try {
      ...
      throw status(
        404,
        `Failed to stop container with ID: ${containerId}; ${err}` satisfies ContainerModel.response
      );
    }
    return this.getContainerInfo(container.id);
  }
  ...
  static async deleteContainer(containerId: string, force: boolean = false) {
    ...
    try {
      ...
      throw status(
        404,
        `Failed to delete container with ID: ${containerId}; ${err}` satisfies ContainerModel.response
      );
    }
  }

  static async listContainers(
    game: ContainerModel.supportedGames | undefined = undefined
  ): Promise<ContainerModel.containerInfo[]> {
    await this.verifyDockerConnection();
    ...
    const containerInfos = containers.map(async (c) => {
      return await this.getContainerInfo(c.Id);
    });
  ...
  static async getLogs(
    ...
    await this.ensureContainerIsManaged(containerId);
    console.info(
      `Getting logs for container with ID: ${containerId}, stdout: ${stdout}, stderr: ${stderr}, tail: ${tail}`
    );
    const container = docker.getContainer(containerId);
    try {
      ...
      throw status(
        404,
        `Failed to get logs for container with ID: ${containerId}; ${err}` satisfies ContainerModel.response
      );
    }
  }
  ...
  static async execInContainer(containerId: string, dockerCmd: string[]) {
    ...
    await this.ensureContainerIsManaged(containerId);
    console.info(
      `Executing command in container with ID: ${containerId}, cmd: ${dockerCmd.join(' ')}`
    );
    const container = docker.getContainer(containerId);
    try {
      ...
      throw status(
        404,
        `Failed to exec command in container with ID: ${containerId}; ${err}` satisfies ContainerModel.response
      );
    }
  }
}
