...
export function ContainerInspectView({ container }: ContainerInspectViewProps) {
  ...
  useEffect(() => {
    ...
    const fetchLogs = async () => {
      ...
      if (result.data) {
        const log = result.data.logs.split('\n').filter((line) => line.trim() !== '');
        setLogs(log);
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">{container.name}</h2>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-end gap-3">
              {isRunning ? (
                ...
                >
                  {!loading ? <IconPlayerPauseFilled className="w-4 h-4" /> : <Spinner />}
                </Button>
              ) : (
                <Button
                  variant="outline"
                ...
                >
                  {!loading ? <IconPlayerPlayFilled className="w-4 h-4" /> : <Spinner />}
                </Button>
              )}
              ...
              <Button
                ...
                size="sm"
                disabled={!isRunning}
                onClick={async () => await invokeCommand((id) => stopContainer(id, true))}
              >
                {!loading ? <IconPlayerStopFilled className="w-4 h-4 text-red-500" /> : <Spinner />}
              </Button>
              <Button
                variant="outline"
                size="sm"
              ...
              >
                {!loading ? <IconTrash className="w-4 h-4 text-red-500" /> : <Spinner />}
              </Button>
            </div>
          </div>
        </div>
        ...
        <TabsContent value="overview" className="space-y-4 mt-4">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <Card>
              ...
              <CardContent>
                <div className="text-sm font-bold truncate" title={container.image}>
                  {container.image}
                </div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-lg font-bold">{container.domain || '-'}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Docker labels attached to this container.</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="environment" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Configuration injected into the container.</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {container.ports.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {container.ports.map((port) => (
                      <Badge key={port} variant="outline" className="font-mono text-sm px-3 py-1">
                        {port}
                      </Badge>
                    ))}
                  </div>
              ...
              <ScrollArea className="h-[400px] w-full rounded-md border p-4 font-mono text-sm">
                {logs.map((log, index) => (
                  <div key={index} className="pb-2">
                    <span className={`text-sm ${getColorFromLog(log)}`}>{log.slice(3)}</span>
                  </div>
                ))}
              </ScrollArea>
            </CardContent>
